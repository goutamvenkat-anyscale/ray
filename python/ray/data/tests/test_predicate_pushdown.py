...
class TestPassthroughWithSubstitutionBehavior:
    ...
    def test_rename_with_partition_residual_filter(
        ...
        ds = (
            ray.data.read_parquet(str(tmp_path), partitioning=Partitioning("hive", field_types={"partition_col": int}))
        ...
        expected = ray.data.read_parquet(
            str(tmp_path), partitioning=Partitioning("hive", field_types={"partition_col": int})
        ).filter(
